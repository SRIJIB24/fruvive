...
$coreSlugs = ['our-story', 'why-fruvive', 'news-blogs'];
...
if (isset($_POST['add_page'])) {
    ...
    if (empty($title) || empty($slug)) {
        ...
        if ($chk->fetchColumn() > 0) {
            ...
            $ins = $object->conn->prepare("INSERT INTO pages_content (client_id, slug, title, content, section) VALUES (1, :slug, :title, :content, 'company')");
            $res = $ins->execute([
                ':slug' => $slug,
                ':title' => $title,
                ':content' => $content
            ...
            if ($res) {
                $_SESSION['msg'] = "Company page '{$title}' created successfully!";
                $_SESSION['msg_type'] = "success";
                header("Location: admincompany.php?page=" . $slug);
                exit();
            } else {
    ...
    header("Location: admincompany.php");
    exit();
}
...
if (isset($_GET['delete'])) {
    ...
    if (in_array($delSlug, $coreSlugs)) {
        ...
        $del = $object->conn->prepare("DELETE FROM pages_content WHERE slug = :slug AND section = 'company' AND client_id = 1");
...
if (isset($_POST['save_page'])) {
    ...
    $up = $object->conn->prepare("UPDATE pages_content SET title = :title, content = :content WHERE slug = :slug AND section = 'company' AND client_id = 1");
    $res = $up->execute([
        ':title' => $page_title,
    ...
    if ($res) {
        $_SESSION['msg'] = "Company page '{$page_title}' updated successfully!";
        $_SESSION['msg_type'] = "success";
    } else {
        $_SESSION['msg'] = "Failed to update page.";
        $_SESSION['msg_type'] = "error";
    }
    header("Location: admincompany.php?page=" . $page_slug);
    exit();
}

// Get selected page slug (default to 'our-story')
$slug = isset($_GET['page']) ? trim($_GET['page']) : 'our-story';

// Fetch selected page details
$stmt = $object->conn->prepare("SELECT * FROM pages_content WHERE slug = :slug AND section = 'company' AND client_id = 1");
$stmt->execute([':slug' => $slug]);
$currentPageDetails = $stmt->fetch(PDO::FETCH_ASSOC);

// Fetch all company pages
$allCompanyStmt = $object->conn->query("SELECT slug, title FROM pages_content WHERE section = 'company' AND client_id = 1 ORDER BY id ASC");
$companyPages = $allCompanyStmt->fetchAll(PDO::FETCH_ASSOC);
...
<head>
    ...
    <title>Manage Company Info - Admin</title>
...
<body class="light">
    ...
    <div class="main">
        ...
        <div class="content">
            ...
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 24px;">
                <div>
                    <h1 style="margin: 0; font-size: 24px;">Company Information Manager</h1>
                    <p style="color: var(--text-muted); font-size: 13px; margin: 4px 0 0 0;">Create and customize dynamic company details, stories, and blogs.</p>
                </div>
                <button type="button" onclick="openAddModal()" class="btn-primary">
                    <span class="material-icons" style="font-size: 18px;">add_circle</span> Add Custom Page
                </button>
            </div>
            ...
            <div class="pages-grid">
                ...
                <div class="list-panel">
                    <div class="page-list-group">
                        <span class="page-list-title">Company Info Pages</span>
                        <?php foreach ($companyPages as $p) { ?>
                            <a href="admincompany.php?page=<?= urlencode($p['slug']) ?>" class="page-item-link <?= $slug === $p['slug'] ? 'active' : '' ?>">
                    ...
                    <?php if ($currentPageDetails) { ?>
                        <form action="" method="POST" style="display: flex; flex-direction: column;">
                            ...
                            <div class="form-row">
                                ...
                                <span style="font-size: 10px; color: var(--text-muted);">URLs for company pages are fixed once created.</span>
                            </div>
                            ...
                            <button type="submit" name="save_page" class="btn-primary" style="align-self: flex-start;">
                                <span class="material-icons" style="font-size: 16px;">save</span> Save Page Changes
                            </button>
                        </form>
                    <?php } else { ?>
    ...
    <div id="addPageModal" style="display: none; position: fixed; inset: 0; background: rgba(0,0,0,0.5); backdrop-filter: blur(4px); z-index: 1000; align-items: center; justify-content: center;">
        <div style="background: var(--bg-card, #fff); width: 100%; max-width: 480px; border-radius: 16px; box-shadow: 0 20px 25px -5px rgba(0,0,0,0.1), 0 10px 10px -5px rgba(0,0,0,0.04); border: 1px solid var(--border); overflow: hidden;">
            <div style="padding: 18px 24px; border-bottom: 1px solid var(--border); display: flex; justify-content: space-between; align-items: center;">
                <h3 style="margin: 0; font-size: 16px; font-weight: bold; color: var(--text-color);">Create Company Page</h3>
            ...
            <div style="padding: 24px;">
                <form action="" method="POST" style="display: flex; flex-direction: column; gap: 16px;">
                    <div class="form-row">
                        <label>Page Title</label>
                        <input type="text" name="title" placeholder="e.g. CSR Activities" required oninput="generateSlug(this.value)">
                    </div>

                    <div class="form-row">
                        <label>URL Slug</label>
                        <input type="text" id="add_slug" name="slug" placeholder="e.g. csr-activities" required style="font-weight: bold;">
                    </div>
    ...
    <script>
        ...
        function confirmDelete(slug) {
            Swal.fire({
                title: 'Are you sure?',
                text: 'This will permanently remove this page from the company profile and sidebar navigation.',
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    window.location.href = 'admincompany.php?delete=' + slug;
                }
            });
        }
