...
$value = [];
if ($query !== '') {
    $value = $object->searchProducts($query);
}
?>
<!DOCTYPE html>
<html lang="en">
...
<head>
    ...
    <title>Search Results: "<?php echo htmlspecialchars($query); ?>" - Fruvive</title>
...
<body class="bg-gray-50 dark:bg-gray-900 text-gray-800 dark:text-gray-200 min-h-screen transition-colors duration-300">
    ...
    <div class="main flex flex-col min-h-screen">
        ...
        <div class="text-center py-12 px-4 bg-gradient-to-b from-green-50/50 to-transparent dark:from-green-950/20">
            <h1 class="text-4xl font-extrabold text-gray-900 dark:text-white tracking-tight">Search Results</h1>
            <p class="max-w-2xl mx-auto mt-3 text-base text-gray-500 dark:text-gray-400">
                <?php if ($query !== '') { ?>
                    Showing results for "<strong><?php echo htmlspecialchars($query); ?></strong>" (<?php echo count($value); ?> items found)
                <?php } else { ?>
        ...
        <main class="flex-grow max-w-7xl mx-auto w-full px-4 md:px-8 pb-16">
                    ...
                    ?>
                        ...
                        <div class="group relative bg-white dark:bg-gray-800 rounded-3xl border border-gray-200/80 dark:border-gray-700/80 p-5 shadow-sm hover:shadow-xl transition duration-300 flex flex-col justify-between">
                            ...
                            <div class="mt-4 pt-4 border-t border-gray-100 dark:border-gray-700/50">
                                ...
                                $countInCart = 0;
                                if ($userid) {
                                    $countInCart = $object->isincart($proid, $userid);
                                }
                                if ($countInCart == 0) {
                                ?>
            ...
            <?php } else { ?>
                <div class="text-center py-16 bg-white dark:bg-gray-800 rounded-3xl border border-gray-200 dark:border-gray-700 p-8 shadow-sm">
                    <span class="material-icons text-6xl text-gray-300 dark:text-gray-600">search_off</span>
                    <h2 class="text-xl font-bold text-gray-700 dark:text-gray-300 mt-4">No Products Found</h2>
                    <p class="text-sm text-gray-500 dark:text-gray-400 mt-2">Try checking spelling or search for another organic product.</p>
                </div>
            <?php } ?>
        </main>
